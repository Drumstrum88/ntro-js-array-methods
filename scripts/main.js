...
const renderCards = (array) => {
  let refStuff = "<h1 class='text-white'></h1>";
...
const search = (event) => {
  ...
  const searchResult = referenceList.filter(items => 
    items.title.toLocaleLowerCase().includes(eventLC) ||
    items.author.toLocaleLowerCase().includes(eventLC) || 
    items.description.toLowerCase().includes(eventLC)
  )
...
const cartTotal = () => {
  ...
  const free = cart.some(item => taco.price <= 0);
